    ...
    import('p5').then((p5) => {
      const Sketch = (p) => {
        ...
        p.setup = () => {
          ...
          for (let i = 0; i < 10; i++) {
            let x = Math.floor(p.random(p.width / gridSize)) * gridSize;
            let y = Math.floor(p.random(p.height / gridSize)) * gridSize;
        ...
        class Square {
          ...
          initiateMove() {
            ...
            this.targetX = this.x + ((Math.random() > 0.5) ? this.size : -this.size) * (Math.random() > 0.5 ? 1 : 0);
            this.targetY = this.y + ((Math.random() > 0.5) ? this.size : -this.size) * (Math.random() <= 0.5 ? 1 : 0);
            // Ensure the target position is within canvas bounds
            this.targetX = Math.max(0, Math.min(this.p.width - this.size, this.targetX));
            this.targetY = Math.max(0, Math.min(this.p.height - this.size, this.targetY));
            this.moveStartTime = this.p.millis();
          }

          update() {
            let elapsedTime = (this.p.millis() - this.moveStartTime) / 1000; // Time for square movement
            if (elapsedTime < 1) {
...
class Cursor {
  ...
  calculateTarget() {
    ...
    this.targetX = this.p.random(this.parentSquare.x + 10, this.parentSquare.x + this.parentSquare.size - 20);
    this.targetY = this.p.random(this.parentSquare.y + 10, this.parentSquare.y + this.parentSquare.size - 20);
    this.currentX = this.targetX; // Update current to target for immediate effect
    this.currentY = this.targetY;
  }

  update() {
    let timeSinceLastMove = this.p.millis() - this.lastMoveTime;
    ...
    let t = Math.min(1, timeSinceLastMove / this.moveDuration); // Ensure t is between 0 and 1
    let easedT = easeInOutCubic(t);
    this.currentX = this.p.lerp(this.parentSquare.x + this.parentSquare.size / 2, this.targetX, easedT);
    this.currentY = this.p.lerp(this.parentSquare.y + this.parentSquare.size / 2, this.targetY, easedT);
  }


  display() {
    // Use currentX and currentY for positioning instead of targetX and targetY
    let safeX = this.p.constrain(this.currentX, this.parentSquare.x + 10, this.parentSquare.x + this.parentSquare.size - 20);
    let safeY = this.p.constrain(this.currentY, this.parentSquare.y + 10, this.parentSquare.y + this.parentSquare.size - 20);
